...
function OptOutForm() {
  ...
  // 🌟 FIX: We use a ref to prevent React StrictMode from firing the API twice!
  ...
  // =====================================================
  useEffect(() => {
    // 🌟 If we already called the API, stop it from calling again
      ...
      try {
        ...
        if (error.response) {
          if (error.response.status === 401) {
            // 🌟 FIX: setTimeout prevents the alert from freezing the UI router
            setTimeout(() => {
